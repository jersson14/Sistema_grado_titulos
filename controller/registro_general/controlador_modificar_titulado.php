<?php
    ...
    if (empty($nombrearchivo)) {
        ...
        if ($nombrearchivo == 'controller/registro_general/documentos/') {
            $ruta = $archivoactual; // Simplemente usa el nombre sin modificarlo
        } else {
            $ruta = 'controller/registro_general/documentos/' . $nombrearchivo; // Construye la ruta completa para la nueva foto
        }
    }
    
    if (!empty($nombrearchivo)) {
        if ($nombrearchivo != 'controller/registro_general/documentos/' && move_uploaded_file($_FILES['achivoobj']['tmp_name'], "documentos/" . $nombrearchivo)) {
