<?php
    ...
    if (empty($nombrearchivo)) {
        ...
        if ($nombrearchivo == 'controller/registro_general_posgrado/documentos/') {
            $ruta = $archivoactual; // Simplemente usa el nombre sin modificarlo
        } else {
            $ruta = 'controller/registro_general_posgrado/documentos/' . $nombrearchivo; // Construye la ruta completa para la nueva foto
        }
    }
    
    if (!empty($nombrearchivo)) {
        if ($nombrearchivo != 'controller/registro_general_posgrado/documentos/' && move_uploaded_file($_FILES['achivoobj']['tmp_name'], "documentos/" . $nombrearchivo)) {
