...
function listar_empleado(){
  tbl_empleado = $("#tabla_empleado").DataTable({
      ...
      buttons: [
        {
          ...
          exportOptions: {
              columns: [0, 2, 3, 4, 5, 6, 7, 8]  // Especifica las columnas a exportar hasta "estado"
          }
        },
        {
          extend: 'pdfHtml5',
      ...
      "columns":[
        ...
        render: function(data,type,row){
          if(data=='CAS'){
          return '<span class="badge bg-success">CAS</span>';
          }else if (data=='Funcionario'){
          return '<span class="badge bg-primary">FUNCIONARIO</span>';
          }else if (data=='Practicante'){
            return '<span class="badge bg-secondary">PRACTICANTE</span>';
          }else if (data=='Servidor Público'){
            return '<span class="badge bg-warning">SERVIDOR PÚBLICO</span>';
          }else if (data=='Tercero'){
            return '<span class="badge bg-dark">TERCERO</span>';
          }
        }   
        },
        {"data":"emple_email"},
...
$('#tabla_empleado').on('click','.editar',function(){
  ...
  document.getElementById('txt_modalidad_editar').value=data.empl_modalidad;
...
function Registrar_Empleado(){
  ...
  let modalidad = document.getElementById('select_modalidad').value;
  let dire = document.getElementById('txt_dire').value;
  let email = document.getElementById('txt_email').value;

  if(nro.length==0 || nom.length==0 || apepa.length==0 || apema.length==0 || fnac.length==0 || movil.length==0 || modalidad.length==0 || dire.length==0 || email.length==0){
...
function Modificar_Empleado(){
  ...
  let modalidad = document.getElementById('txt_modalidad_editar').value;
  ...
  if(id.length==0 || nro.length==0 || nom.length==0 || apepa.length==0 || apema.length==0 || fnac.length==0 || movil.length==0 || modalidad.length==0 || dire.length==0 || email.length==0 || esta.length==0){
      return Swal.fire("Mensaje de Advertencia","Tiene campos vacios","warning");
  }
  if(validar_email(email)){
...
$('#tabla_empleado').on('click','.foto',function(){
  ...
  document.getElementById('lb_usuario').innerHTML=data.emple_nombre+" "+data.emple_apepat+" "+data.emple_apemat;
...
function Modificar_Foto_Empleado(){
  ...
  if(id.length==0 || foto.length==0){
    return Swal.fire("Mensaje de Advertencia","Debe seleccionar una foto","warning");
}
