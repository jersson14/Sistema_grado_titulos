...
function listar_estudiantes() {
  tbl_estudiantes = $("#tabla_estudiantes").DataTable({
    ...
    processing: true,
    ajax: {
      url: "../controller/estudiantes/controlador_listar_estudiantes.php",
      type: "POST",
    },
    dom: "Bfrtip",
    ...
    buttons: [
      {
        ...
        filename: function () {
          return "LISTA DE ESTUDIANTES PREGRADO";
        },
        title: function () {
          return "LISTA DE ESTUDIANTES PREGRADO";
        },
    ...
    columns: [
      ...
      {
        defaultContent:
          "<button class='ver btn btn-info btn-sm' title='Ver información'><i class='fa fa-eye'></i> Ver</button> <button class='editar btn btn-primary btn-sm' title='Editar datos'><i class='fa fa-edit'></i> Editar</button>",
      },
    ],
...
// ⭐ NUEVO: Evento click en botón "Ver"
...
$("#tabla_estudiantes").on("click", ".editar", function () {
  ...
  // ⭐ NUEVO: Cargar foto actual si existe
  if (
    data.foto_pasaporte &&
    data.foto_pasaporte !== "" &&
    data.foto_pasaporte !== null
  ...
  // Limpiar input de foto y preview
  $("#txt_foto_editar").val("");
...
// ⭐ NUEVO: Preview de foto antes de subir
...
function Modificarr_Estudiante() {
  ...
  // Validar solo campos obligatorios (dirección y fechas son opcionales)
  if (
    dni.length == 0 ||
    nombres.length == 0 ||
    apepa.length == 0 ||
  ...
  // ⭐ NUEVO: Preparar FormData para enviar archivo
  ...
  $.ajax({
    url: "../controller/estudiantes/controlador_modificar_estudiantes.php",
    type: "POST",
    data: formData,
...
function CambiarEstatusEtnica(dni, estatus) {
  $.ajax({
    url: "../controller/estudiantes/controlador_modificar_estatus_etnica.php",
    type: "POST",
