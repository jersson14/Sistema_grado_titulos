...
function listar_expedientes_bachiller() {
  tbl_general_bachiller = $("#tabla_registro_general_bachiller").DataTable({
    ordering: false,
    bLengthChange: true,
    ...
    processing: true,
    ajax: {
      url: "../controller/registro_general_bachiller/controlador_listar_registro_general.php",
      type: "POST",
    },
    dom: "Bfrtip",
    buttons: [
      {
        ...
        filename: function () {
          return "LISTA GENERAL DE BACHILLERES";
        },
        title: function () {
          return "LISTA GENERAL DE BACHILLERES";
        },
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          if (
            data == "controller/registro_general_bachiller/documentos/" ||
            data == ""
          ) {
  ...
  tbl_general_bachiller.on("draw.td", function () {
    var PageInfo = $("#tabla_registro_general_bachiller")
      .DataTable()
      .page.info();
    tbl_general_bachiller
...
function listar_expedientes_alfabetico() {
  tbl_general_bachiller = $("#tabla_registro_general_bachiller").DataTable({
    ...
    processing: true,
    ajax: {
      url: "../controller/registro_general_bachiller/controlador_listar_registro_alfabetico.php",
      type: "POST",
    },
    dom: "Bfrtip",
    buttons: [
...
function listar_fechas_busqueda() {
  ...
  tbl_general_bachiller = $("#tabla_registro_general_bachiller").DataTable({
    ...
    processing: true,
    ajax: {
      url: "../controller/registro_general_bachiller/controlador_listar_registro_general_bachiller_fecha.php",
      type: "POST",
      data: {
...
$("#tabla_registro_general_bachiller").on("click", ".mostrar", function () {
  ...
  document.getElementById("txt_bachiller_mas").value = data.Grado_bachiller_de;
...
//EDITAR
$("#tabla_registro_general_bachiller").on("click", ".editar", function () {
  var data = tbl_general_bachiller.row($(this).parents("tr")).data();
  ...
  //DATOS DEL TITULADO  document.getElementById('txt_tipo_documento_editar').value=data.tipo_documento;
  ...
  document.getElementById("txt_bachiller_editar").value =
    data.Grado_bachiller_de;
  document.getElementById("txt_modalidad_editar").value =
    data.Modalidad.replace(/&QUOT;/g, '"');
...
//AGREGAR DIPLOMA
$("#tabla_registro_general_bachiller").on("click", ".agregar", function () {
  var data = tbl_general_bachiller.row($(this).parents("tr")).data();
...
function Cargar_Select_Carrera_editar() {
  $.ajax({
    ...
    .done(function (resp) {
      console.log("Respuesta AJAX: ", resp); // Depurar la respuesta del servidor
      let data = JSON.parse(resp);
      let cadena = "<option value=''>Seleccione</option>"; // Agrega la opción "Seleccione" al inicio
      ...
      if (data.length > 0) {
        ...
        console.log("ID seleccionado: ", id); // Depurar el valor seleccionado
        Traergrado(id);
      } else {
        let cadena = "<option value=''>No hay tipos disponibles</option>";
        document.getElementById("txt_escuela_editar").innerHTML = cadena;
      }
    })
    .fail(function () {
...
function Traergrado(idgrado) {
  console.log("Cargando datos para el ID: ", idgrado);
  ...
  $.ajax({
    ...
    .done(function (resp) {
      console.log("Respuesta Traergrado: ", resp);
      var data = JSON.parse(resp);

      if (data.length > 0) {
        console.log("Datos recibidos: ", data);
        $("#txt_bachiller").val(data[0][1]);
        $("#txt_bachiller_editar").val(data[0][1]);
      } else {
        console.log("No se encontraron datos.");
        $("#txt_bachiller").val("");
        $("#txt_bachiller_editar").val("No se encontraron registros");
      }
    })
    .fail(function () {
      alert("Error al cargar la información de grado");
    });
}
...
function Agregar_diploma() {
  ...
  let nrocred = parseInt(document.getElementById("txt_nro_credi").value) || 0;
  ...
  let ciclo_inves = document.getElementById("select_ciclo_investigacion").value;
  ...
  $.ajax({
    ...
    if (resp.status) {
      ...
      var url =
        "../view/MPDF/REPORTE/diploma_bachiller.php?codigo=" +
        id +
        "&tamaño=55&tamaño2=93#zoom=100%";
...
function Registrar_Bachiller() {
  ...
  let bach = document.getElementById("txt_bachiller").value;
...
function limpiarCampos() {
  ...
  document.getElementById("txt_bachiller").value = "";
...
function Modificar_Bachiller() {
  ...
  let bach = document.getElementById("txt_bachiller_editar").value;
...
$("#tabla_registro_general_bachiller").on("click", ".delete", function () {
  var data = tbl_general_bachiller.row($(this).parents("tr")).data();
...
function Imprimir_informe() {
  ...
  $.ajax({
    url: "../view/MPDF/REPORTE/informe_por_escuela_bach.php",
    type: "POST",
    data: {
      info: numero,
      grado: grado1,
          ...
          $.ajax({
            ...
            success: function (resp) {
              ...
              if (resp == 1) {
                ...
                window.open(
                  "https://gradosapp.fun/view" +
                    encodeURIComponent(pdfResult.filepath),
                  "Informe por Escuela",
                  "scrollbars=NO,width=" +
                    screen.width +
                    ",height=" +
...
function Imprimir_informe_facul() {
  ...
  var infoValue = parseInt(numero, 10) || 0; // Obtener el valor actual de txt_info
  var info = infoValue.toString();
  var grado = document.getElementById("txt_gr").value;
  var fac = document.getElementById("txtfacu").value;
  var fedes = document.getElementById("fechadesde").value;
  var fehas = document.getElementById("fechahasta").value;
  ...
  var url =
    "../view/MPDF/REPORTE/informe_por_facultad_bach.php?" +
    "info=" +
  ...
  // Realizar la petición AJAX para actualizar el número del informe
  $.ajax({
    ...
    success: function (resp) {
      if (resp > 0) {
        // Abrir el informe solo si la actualización del número fue exitosa
        window.open(
          url,
...
//IMPRIMIR DIPLOMA
$("#tabla_registro_general_bachiller").on("click", ".diploma", function () {
  var data = tbl_general_bachiller.row($(this).parents("tr")).data();
  ...
  var url =
    ...
    "&tamaño=" +
    encodeURIComponent(tamaño) +
    "&tamaño2=" +
    encodeURIComponent(tamaño2) +
    "#zoom=100%";
...
$("#tabla_registro_general_bachiller").on("click", ".letra", function () {
  var data = tbl_general_bachiller.row($(this).parents("tr")).data();
...
function imprimir_numero() {
  ...
  if (codigo.length == 0 || tamaño.length == 0) {
    ...
    var url =
      "../view/MPDF/REPORTE/diploma_bachiller.php?" +
      "codigo=" +
...
function Total_diploma_bachiller() {
  $.ajax({
    url: "../controller/registro_general_bachiller/controlador_total_diploma_bachiller.php",
    type: "POST",
  }).done(function (resp) {
    var data = JSON.parse(resp);
    var cadena = "";
    if (data.length > 0) {
      $("#total_diplo_bachiller").html(data[0][0]);
    } else {
      return Swal.fire(
        "Mensaje de Error",
...
function Total_diploma_titulo() {
  $.ajax({
    url: "../controller/registro_general/controlador_total_diploma_titulo.php",
    type: "POST",
  }).done(function (resp) {
    var data = JSON.parse(resp);
    var cadena = "";
    if (data.length > 0) {
      $("#total_diplo_titulo").html(data[0][0]);
    } else {
      return Swal.fire(
        "Mensaje de Error",
        "No se pudo traer los resultados",
...
function Total_expediente_bach() {
  $.ajax({
    url: "../controller/registro_general_bachiller/controlador_total_expe_bach.php",
    type: "POST",
  }).done(function (resp) {
    var data = JSON.parse(resp);
    var cadena = "";
    if (data.length > 0) {
      $("#totalexpe_bachiller").html(data[0][0]);
    } else {
      return Swal.fire(
        "Mensaje de Error",
        "No se pudo traer los resultados",
...
function Total_expediente_titu() {
  $.ajax({
    url: "../controller/registro_general/controlador_total_expe_titu.php",
    type: "POST",
  }).done(function (resp) {
    var data = JSON.parse(resp);
    var cadena = "";
    if (data.length > 0) {
      $("#totalexpe_titulo").html(data[0][0]);
    } else {
      return Swal.fire(
        "Mensaje de Error",
        "No se pudo traer los resultados",
...
function Total_pregrado() {
  $.ajax({
    url: "../controller/registro_general/controlador_total_pregrado.php",
    type: "POST",
  }).done(function (resp) {
    var data = JSON.parse(resp);
    var cadena = "";
    if (data.length > 0) {
      $("#total_pregrado").html(data[0][0]);
    } else {
      return Swal.fire(
        "Mensaje de Error",
        "No se pudo traer los resultados",
...
function Imprimir_informe2() {
  ...
  if (infoValue.length > 0) {
    ...
    var url =
      "../view/MPDF/REPORTE/informe_por_filial_bachiller.php?" +
      "info=" +
      encodeURIComponent(info) +
      "&grado=" +
      encodeURIComponent(grado) +
...
$("#tabla_registro_general_bachiller").on("click", ".pasar", function () {
  var data = tbl_general_bachiller.row($(this).parents("tr")).data();

  if (tbl_general_bachiller.row(this).child.isShown()) {
    var data = tbl_general_bachiller.row(this).data();
  }
  $("#modal_pasar_titulado").modal("show");
  document.getElementById("lb_titulo_datos22").innerHTML =
    "<b>DNI: </b>" + data.Dni;
  document.getElementById("lb_titulo_datos33").innerHTML =
    "<b>ESTUDIANTE: </b>" + data.Estudiante;
  document.getElementById("lb_titulo_datos44").innerHTML =
    "<b>FACULTAD - ESCUELA PROFESIONAL: </b>" +
    data.Facultad +
    " - " +
    data.Escuela;
  document.getElementById("txtidexpe").value = data.Id_expediente;
  document.getElementById("select_sexo_pasar").value = data.Sexo;
});
...
function Traergrado2(idgrado) {
  $.ajax({
    ...
    if (data.length > 0) {
      ...
      var genero = $("#select_sexo_pasar").val(); // Asume que tienes un select con id "select_sexo"
      ...
      // Asigna el título a los inputs
      $("#txt_titulo_pasar").val(titulo);
    } else {
      $("#txt_titulo_pasar").val("No se encontraron registros");
    }
  });
}

// FUNCIÓN PARA PASAR ESTUDIANTE A BACHILLER
function Pasar_titulo() {
  ...
  let grado = document.getElementById("txt_titulo_pasar").value;
  ...
  let nombreEstudiante =
    document.getElementById("lb_titulo_datos33").textContent || "el estudiante";

  $.ajax({
    url: "../controller/registro_general_bachiller/controlador_pasar_titulado.php",
    type: "POST",
    data: {
      idexpe: idexpe,
      idautori: idautori,
    ...
    if (resp > 0) {
      Swal.fire({
        title: `¿Está seguro de enviar los datos del estudiante ${nombreEstudiante} a los titulados?`,
        text: "Una vez aceptado, se enviará este registro a titulados y ya no estará visible en esta sección.",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        ...
        if (result.isConfirmed) {
          tbl_general_bachiller.ajax.reload();
          $("#modal_pasar_titulado").modal("hide");

          // Mensaje de satisfacción
          Swal.fire({
            title: "Operación Exitosa",
            text: `El estudiante ${nombreEstudiante} ha sido registrado como titulado exitosamente.`,
            icon: "success",
...
//ENVIANDO AL BOTON DELETE
$("#tabla_registro_general_bachiller").on("click", ".validar", function () {
  var data = tbl_general_bachiller.row($(this).parents("tr")).data();

  if (tbl_general_bachiller.row(this).child.isShown()) {
    data = tbl_general_bachiller.row(this).data();
  }

  Swal.fire({
    title:
      "¿Desea validar el expediente del bachiller: " + data.Estudiante + "?",
...
function imprimir_varios_diplomas() {
  var id_colacion = document
    .getElementById("select_fechacola_varios")
    .value.toString();

  var registro_inicio = document
    .getElementById("txt_registro_inicio")
    .value.toString();
  var registro_fin = document
    .getElementById("txt_registro_fin")
    .value.toString();

  if (
    id_colacion.length == 0 ||
    registro_inicio.length == 0 ||
    registro_fin.length == 0
  ) {
    return Swal.fire(
      "Mensaje de Advertencia",
    ...
    var url =
      "../view/MPDF/REPORTE/varios_diplomas.php?" +
      "id_colacion=" +
      encodeURIComponent(id_colacion) +
      "&registro_inicio=" +
      encodeURIComponent(registro_inicio) +
      "&registro_fin=" +
      encodeURIComponent(registro_fin) +
      "#zoom=100%";
