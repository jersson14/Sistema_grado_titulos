...
$("#tabla_registro_general_posgrado").on("click", ".editar", function () {
  ...
  $("#select_programa_editar")
    .select2()
    .val(data.Id_programa)
  ...
  document.getElementById("txt_centro_inve_editar").value =
    data.Centro_investigacion;
  document.getElementById("txt_proceden_pais_editar").value =
    data.Proce_pais_ext;
  document.getElementById("txt_procedeni_univer_editar").value =
    data.Proce_univ_ext;
  document.getElementById("txt_proceden_grado_editar").value =
    data.Proce_grado_ext;
...
function Agregar_diploma() {
  ...
  if (nrocred < 48) {
    return Swal.fire(
      "Mensaje de Advertencia",
      "El número de créditos no puede ser menor a 48 creditos",
      "warning",
    );
  }
...
function Modificar_Posgraduado() {
  ...
  let proce_grado = document.getElementById(
    "txt_procedeni_univer_editar",
  ).value;
...
function Eliminar_Expediente(id) {
  $.ajax({
    ...
    if (resp > 0) {
      Swal.fire(
        "Mensaje de Confirmación",
        "Se elimino el registro del posgraduado con exito",
        "success",
      ).then((value) => {
        tbl_general_posgrado.ajax.reload();
      });
      ...
      return Swal.fire(
        ...
        "No se puede eliminar este registro por que esta siendo utilizado en otros formularios, verifique por favor",
        "warning",
      );
    }
  });
...
function Imprimir_informe() {
  ...
  var url =
    "../view/MPDF/REPORTE/informe_por_maestria.php?" +
    "info=" +
...
function imprimir_numero() {
  ...
  if (codigo.length == 0 || tamaño.length == 0) {
    ...
    var url =
      "../view/MPDF/REPORTE/maestria.php?" +
      "codigo=" +
  ...
  gapi.client.sheets.spreadsheets.values
    ...
    .then(function (response) {
      ...
      if (data.length > 0) {
        ...
        for (var i = 0; i < data.length; i++) {
          var fila = data[i];
          if (fila[1] === dniIngresado || fila[1] === dniIngresado2) {
