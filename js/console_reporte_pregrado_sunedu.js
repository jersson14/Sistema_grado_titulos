...
function listar_expedientes_pregrado_fecha() {
  ...
  tbl_general_pregrado = $("#tabla_registro_general_pregrado").DataTable({
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          if (
            data == "controller/registro_general/documentos/" ||
      ...
      {
        data: function (row) {
          return row.Abreviatura_grado === "B"
            ? "Bachiller en " + row.Grado_bachiller_de
            : "Titulo Profesional de " + row.Titulo_de;
        },
        visible: false,
      },
      { data: null, defaultContent: "", visible: false }, // Columna vacía
  ...
  tbl_general_pregrado.on("draw.td", function () {
    var PageInfo = $("#tabla_registro_general_pregrado")
      .DataTable()
      .page.info();
    tbl_general_pregrado
