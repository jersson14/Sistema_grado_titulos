...
<head>
  ...
  <title>UTEA | PERÚ - Posgrado</title>
  ...
  <style>
    * {
    ...
    /* Fondo animado de graduación académica - ESPECÍFICO PARA POSGRADO */
    .animated-background {
      position: fixed;
      top: 0;
      left: 0;
    ...
    .header-section::before {
      ...
      right: 0;
      height: 5px;
      background: linear-gradient(90deg, #0065B2, #7e22ce, #0065B2);
  ...
  <div class="container-fluid">
    ...
    <div class="header-section">
      ...
      <div class="header-subtitle">
        ...
        <span>Seguimiento de Expediente Posgrado</span>
        <span class="badge-posgrado">POSGRADO</span>
      </div>
      
      <div class="nav-links">
        <a href="seguimiento.php" class="nav-btn pregrado">
          <i class="fa fa-university"></i>
          <span>Seguimiento Pregrado</span>
        </a>
    ...
    <div class="main-card">
      <div class="info-banner">
        <p><strong>Bienvenido al Sistema de Seguimiento de Expedientes de Posgrado.</strong> La Universidad Tecnológica de los Andes pone a su disposición esta plataforma especializada para consultar el estado de su expediente de posgrado de manera rápida y segura. Para una búsqueda exitosa, ingrese su <strong>Número de Documento</strong> (DNI, Pasaporte o Carnet de Extranjería) sin espacios ni guiones y luego de clic en Buscar Expediente Posgrado.</p>
      </div>
      ...
      <div class="search-section">
        <h2 class="search-title">
          ...
          <span>Buscar Expediente de Posgrado</span>
        </h2>
        
        <div class="search-form">
          <div class="form-group">
            <label class="form-label">
              Número de Documento del Posgraduado <span class="required">(*)</span>
            </label>
            <input 
              type="text" 
              class="form-control" 
          ...
          <div class="form-group">
            <button class="btn-search" id="buscar" onclick="Traer_Datos_Seguimiento_posgrado()">
              <i class="fa fa-search"></i>
              <span>Buscar Expediente Posgrado</span>
            </button>
          </div>
        </div>
      </div>
...
function Traer_Datos_Seguimiento_posgrado() {
  ...
  $.ajax({
    ...
    if (data.length > 0) {
      ...
      $.ajax({
        url: "controller/usuario/controlador_traer_seguimiento_detalle_posgrado.php",
        type: "POST",
        ...
        if (datadetalle.length > 0) {
          datadetalle.sort((a, b) => new Date(b.fecha_larga) - new Date(a.fecha_larga));
      ...
      Swal.fire({
        ...
        html: "No se encontraron datos del documento ingresado.<br><br>Si está buscando un <b>expediente de Pregrado</b>, haga clic en el botón <b>'Seguimiento Pregrado'</b> en la parte superior.",
        confirmButtonColor: "#0065B2",
      ...
      ">
        <strong style="color: #333; font-size: 14px; display: block; line-height: 1.5;">
          ${ultimoEstado === 'COLACION' ? 
            ...
            '📋 Su expediente ha sido registrado y está pendiente de revisión'}
        </strong>
      </div>
    </div>
  `;
  ...
  let infoEstudiante = `
    ...
    ">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 12px;">
        ...
        <div>
          <small style="opacity: 0.9; display: block; margin-bottom: 4px; font-size: 11px;">Programa:</small>
          <strong style="font-size: 14px; display: block; word-wrap: break-word;">${datos.posgrado}</strong>
        </div>
      </div>
    </div>
  `;
        ...
        <table id="tabla_registro_general" class="table table-striped table-bordered" style="width:100%; font-size: 12px;">
          <thead style="background: linear-gradient(135deg, #0065B2, #00a8ff); color: white;">
            <tr>
              ...
              <th style="text-align:center; padding: 10px 8px; font-size: 11px;">Programa</th>
    ...
    columns: [
      {
        ...
        render: function(data, type, row) {
          if (data == "MAESTRIA") {
            return '<span class="badge" style="background: linear-gradient(135deg, #10b981, #059669); color: white; padding: 5px 10px; border-radius: 15px; font-weight: 600; font-size: 10px; white-space: nowrap;"><i class="fas fa-user-graduate"></i> MAESTRÍA</span>';
          } else if (data == "DOCTORADO") {
            return '<span class="badge" style="background: linear-gradient(135deg, #7e22ce, #6b21a8); color: white; padding: 5px 10px; border-radius: 15px; font-weight: 600; font-size: 10px; white-space: nowrap;"><i class="fas fa-graduation-cap"></i> DOCTORADO</span>';
          } else {
            return '<span class="badge" style="background: linear-gradient(135deg, #1e3c72, #2a5298); color: white; padding: 5px 10px; border-radius: 15px; font-weight: 600; font-size: 10px; white-space: nowrap;"><i class="fas fa-certificate"></i> SEGUNDA ESPECIALIDAD</span>';
          }
        }
      },
      { 
    ...
    buttons: [
      {
        ...
        filename: "Expediente_Posgrado_UTEA_" + new Date().toISOString().slice(0,10),
        title: "EXPEDIENTE POSGRADO - UTEA"
      },
      {
        extend: "pdfHtml5",
